...
(async function () {
    ...
    const isPdf =
        ...
        url.endsWith(".pdf") ||
        url.includes(".pdf?") ||
        (!!document.getElementById("viewer") && !!document.getElementById("outerContainer")); // PDF.js viewer

    if (isPdf) {
        console.log("[EdgeTranslate] PDF detected, skipping select.js");
        return; // early exit inside IIFE
    }
